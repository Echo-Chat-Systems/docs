...
Retrieves a [**server-signed user certificate**](/docs/protocol/definitions/user/certificates#server-signed-user-certificate) by their user ID (Ed25519 public key).
This action does not require authentication and returns only non-sensitive information.
...
<ParamsList>
    <Param name="user" type="object" required>
        [Server-Signed User Certificate](/docs/protocol/definitions/user/certificates#server-signed-user-certificate) requested.
    </Param>
</ParamsList>
