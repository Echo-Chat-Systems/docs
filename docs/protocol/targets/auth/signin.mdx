...
<ParamsList>
    <Param name="sign-challenge" type="string" required>
        Random base64 string the user must decode and sign.
    </Param>
    <Param name="encrypt-challenge" type="string" required>
        Random base64 string encrypted using the users public `ek`.
    </Param>
</ParamsList>
...
| `not-found`         | The provided public key was not found in the database                   | The user does not have an account or is trying to sign on to the wrong server    |
