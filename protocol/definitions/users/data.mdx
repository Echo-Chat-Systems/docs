...
| `tag`         | Numeric tag used to disambiguate usernames (required) | –         |
| `pronouns`    | User-specified pronouns                               | –         |
| `bio`         | Freeform biography text                               | –         |
| `css`         | Optional CSS code used for profile theming            | –         |
| `pfp`         | URI to the profile picture                            | –         |
| `banner`      | URI to the profile banner                             | –         |
| `timezone`    | User’s preferred timezone string                      | –         |
| `status`      | Object describing the user’s current status           | –         |
| `status.type` | Type of the status (e.g. "online", "away")            | –         |
...
| Field     | Description                   | Reference                                                  |
|-----------|-------------------------------|------------------------------------------------------------|
| `origin`  | User origin server details.   | [`origin`](/protocol/definitions/meta/origin.mdx)              |
| `keys`    | User public keys              | [`pubkeys`](/protocol/definitions/crypto/keys.mdx#pubkeys)     |
| `profile` | User profile                  | [`profile`](#profile-structure)                            |
| `rep`     | User public reputation record | [`rep`](/protocol/definitions/users/reputation.mdx#rep-object) |
...
| `profile`  | User profile                                                  | [`profile`](#profile-structure)                                |
| `rep`      | User public reputation record                                 | [`rep`](/protocol/definitions/users/reputation.mdx#rep-object) |
| `settings` | Encrypted b64 encoded string containing user roaming settings | -                                                              |
